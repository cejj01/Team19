...
if(isset($_POST['probNum'])){
	...
	if (isset($_POST['rejectNotes'])) {
		...
		$Number = 1
		$sqlReturnToOperator = "UPDATE ProblemNumber SET Notes = '".$rejectNotes."', ReturnToOperator = $Number WHERE ProblemNo = ".$ProblemID;
		...
		$sqlAddSolution = "UPDATE ProblemNumber SET Solution = '".$solution."', Resolved = $Number, ReturnToOperator = $Number WHERE ProblemID = ".$ProblemID;
		...
		//$sqlGetProblemDetails = "SELECT ProblemID, ProblemType, ProblemPriority, SerialNumber, SoftwareName, OS, ProblemDescription, Notes FROM ProblemNumber WHERE ProblemID = ".$ProblemID;
